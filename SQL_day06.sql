...
select ulke , count(id) as calisan_sayisi from insan group by ulke ;
...
select ulke , cinsiyet , count(id) as calisan_sayisi , round(avg(yas)) as yas_ortalamasi from insan group by ulke , cinsiyet;


-- SORU07: maas'ı 30000 den büyük olanların, ulke'lere göre ve cinsiyet'e göre çalışan sayısını, yas ortalamasını listeleyiniz.
	select ulke , cinsiyet , count(isim) as calisan_sayisi , round(avg(yas)) as yas_ortalamasi  from insan
	where maas>30000
...
select ulke , round(avg(yas)) as yas_ortalamasi from insan group by ulke having avg(yas)>30;
	 

--SORU9: ulke'lere göre toplam maası bulup, toplam maas 30000'den buyuk olanları listeleyiniz
select ulke , sum(maas) as toplam_maas from insan group by ulke having sum(maas)>30000;
...
--personel_bilgi tablosundan da id'si 123 olan, personelin tel ve cocuk sayisini yazdirin  
------------------------------------------------------------------------------*/  
